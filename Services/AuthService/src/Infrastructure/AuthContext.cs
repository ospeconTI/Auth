...
namespace OSPeConTI.Auth.Services.Infrastructure
{
    ...
    public class AuthContextDesignFactory : IDesignTimeDbContextFactory<AuthContext>
    {
        ...
        public AuthContext CreateDbContext(string[] args)
        {
            ...
            if (env != "Prod" && env != "Dev")
            {
                throw new Exception("Indique el entorno (\"Prod\" para produción o \"Dev\" para Desarrollo, ejemplo: dotnet ef database update -s ../application --context AuthContext -- \"Prod\")");
            }

            if (env == "Prod")
            {
                var optionsBuilder = new DbContextOptionsBuilder<AuthContext>()
                .UseSqlServer("Server=localhost;Initial Catalog=Auth;User ID=sa;password=oSpEcONSQL3578951");
